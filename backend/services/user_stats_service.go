...
func (s *UserStatsService) updateAccuracyRate(userStats *models.UserStats, quizService *QuizService) {
	...
	todayQuizzes, err := quizService.GetUserTodayQuizzes(userStats.ID)
	if err != nil {
		// 如果获取失败，跳过更新
		return
	}
...
func (s *UserStatsService) updateErrorByCategory(userStats *models.UserStats, category string) {
	for i := range userStats.ErrorDistribution.DataByCategory {
		if userStats.ErrorDistribution.DataByCategory[i].Type == category {
...
func (s *UserStatsService) updateErrorByDifficulty(userStats *models.UserStats, difficulty string) {
	for i := range userStats.ErrorDistribution.DataByDifficulty {
		if userStats.ErrorDistribution.DataByDifficulty[i].Type == difficulty {
...
func (s *UserStatsService) calculateCategoryPercentages(userStats *models.UserStats) {
	...
	// 计算各分类的百分比
	for i := range userStats.ErrorDistribution.DataByCategory {
		percentage := float64(userStats.ErrorDistribution.DataByCategory[i].Count) / float64(totalCategoryErrors) * 100
...
func (s *UserStatsService) calculateDifficultyPercentages(userStats *models.UserStats) {
	...
	// 计算各难度的百分比
	for i := range userStats.ErrorDistribution.DataByDifficulty {
		percentage := float64(userStats.ErrorDistribution.DataByDifficulty[i].Count) / float64(totalDifficultyErrors) * 100
